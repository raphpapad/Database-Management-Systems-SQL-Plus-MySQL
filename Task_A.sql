...
EXPLAIN PLAN SET STATEMENT_ID = 'Z2_A1' FOR 
SELECT p.personID FROM movie m, plays p 
WHERE p.movieID = m.movieID and m.movieID = 0046790;
SELECT ID||' '||PARENT_ID||' '||LPAD(' ', 2*(LEVEL-1))||OPERATION||' '||OPTIONS||' '||OBJECT_NAME "QUERY PLAN" FROM PLAN_TABLE 
WHERE STATEMENT_ID='Z2_A1' START WITH ID = 0 CONNECT BY PRIOR ID=PARENT_ID;
...
EXPLAIN PLAN 
SET STATEMENT_ID = 'Z2_A2' FOR SELECT DISTINCT p.personID
FROM movie m, plays p WHERE p.movieID = m.movieID and m.movieID = 0046790;
SELECT ID||' '||PARENT_ID||' '||LPAD(' ', 2*(LEVEL-1))||OPERATION||' '||OPTIONS||' '||OBJECT_NAME "QUERY PLAN" FROM PLAN_TABLE 
WHERE STATEMENT_ID='Z2_A2' START WITH ID = 0 CONNECT BY PRIOR ID=PARENT_ID;
...
EXPLAIN PLAN SET STATEMENT_ID = 'Z2_D1' FOR 
SELECT movieTitle FROM movie WHERE year > 1980;
SELECT ID||' '||PARENT_ID||' '||LPAD(' ', 2*(LEVEL-1))||OPERATION||' '||OPTIONS||' '||OBJECT_NAME "QUERY PLAN" FROM PLAN_TABLE 
WHERE STATEMENT_ID='Z2_D1' START WITH ID = 0 CONNECT BY PRIOR ID=PARENT_ID;
...
EXPLAIN PLAN SET STATEMENT_ID = 'Z2_D2' FOR 
SELECT personName FROM people WHERE birthYear > 1950;
SELECT ID||' '||PARENT_ID||' '||LPAD(' ', 2*(LEVEL-1))||OPERATION||' '||OPTIONS||' '||OBJECT_NAME "QUERY PLAN" FROM PLAN_TABLE 
WHERE STATEMENT_ID='Z2_D2' START WITH ID = 0 CONNECT BY PRIOR ID=PARENT_ID;
...
EXPLAIN PLAN SET STATEMENT_ID = 'Z2_Ε1' FOR 
SELECT movieID, movieTitle FROM movie WHERE movieID != 0046778 and year > 1980;
SELECT ID||' '||PARENT_ID||' '||LPAD(' ', 2*(LEVEL-1))||OPERATION||' '||OPTIONS||' '||OBJECT_NAME "QUERY PLAN" FROM PLAN_TABLE 
WHERE STATEMENT_ID='Z2_Ε1' START WITH ID = 0 CONNECT BY PRIOR ID=PARENT_ID;
...
EXPLAIN PLAN SET STATEMENT_ID = 'Z2_Ε2' FOR 
SELECT movieID, movieTitle FROM movie WHERE movieID = 0046778 and year > 1980;
SELECT ID||' '||PARENT_ID||' '||LPAD(' ', 2*(LEVEL-1))||OPERATION||' '||OPTIONS||' '||OBJECT_NAME "QUERY PLAN" FROM PLAN_TABLE 
WHERE STATEMENT_ID='Z2_Ε2' START WITH ID = 0 CONNECT BY PRIOR ID=PARENT_ID;
...
EXPLAIN PLAN SET STATEMENT_ID = 'Z2_F' FOR 
SELECT m.movieID, m.movieTitle, p.companyID FROM movie m, producedBy p WHERE m.movieID = p.movieID and m.movieID = 0046790 and m.movieTitle = 'Boot Polish (1954)';
SELECT ID||' '||PARENT_ID||' '||LPAD(' ', 2*(LEVEL-1))||OPERATION||' '||OPTIONS||' '||OBJECT_NAME "QUERY PLAN" FROM PLAN_TABLE 
WHERE STATEMENT_ID='Z2_F' START WITH ID = 0 CONNECT BY PRIOR ID=PARENT_ID;
...
EXPLAIN PLAN SET STATEMENT_ID = 'Z2_G1' FOR 
SELECT DISTINCT pl.movieID FROM people p, plays pl WHERE p.personID = pl.personID and p.birthYear > 1990;
SELECT ID||' '||PARENT_ID||' '||LPAD(' ', 2*(LEVEL-1))||OPERATION||' '||OPTIONS||' '||OBJECT_NAME "QUERY PLAN" FROM PLAN_TABLE 
WHERE STATEMENT_ID='Z2_G1' START WITH ID = 0 CONNECT BY PRIOR ID=PARENT_ID;
